...
export class CategoriesService extends BaseApiService<Category> {
    ...
    public getAllCategories(): Observable<Array<Category>> {
        return this.getAll('api/categories', this._databaseService.categories,
            Observable.fromPromise(
                this._databaseService.categories.filter(c => c.state !== ModelState.Deleted).sortBy('name')));
    }
    ...
    public deleteCategory(id: string): Observable<boolean> {
        return this.deleteItem(`api/categories/${id}`,
            this.getDeleteOfflineFallback(id));
    }
    ...
    private getDeleteOfflineFallback(id: string): Observable<boolean> {
        return Observable.fromPromise(this._databaseService.categories.update(id, {state: ModelState.Deleted})
            .then(() => {
                return true;
            }, () => {
                return false;
