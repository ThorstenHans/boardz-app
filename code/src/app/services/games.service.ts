...
export class GamesService extends BaseApiService<Game> {
    ...
    public getAllGames(): Observable<Game[]> {
        return this.getAll('api/games',
            ...
            Observable.fromPromise(this._databaseService.games.filter(g => g.state !== ModelState.Deleted).sortBy('name')));
    }
    ...
    public deleteGame(id: string): Observable<boolean> {
        return this.deleteItem(`api/games/${id}`, this.getDeleteOfflineFallback(id));
    }

    private getUpdateOfflineFallback(game: Game): Observable<boolean> {
        const copy = this.deepClone(game);
        if (copy.state !== ModelState.New) {
    ...
    private getDeleteOfflineFallback(id: string): Observable<boolean> {
        return Observable.fromPromise(this._databaseService.games.update(id, {state: ModelState.Deleted})
            .then(() => {
                return true;
            }, () => {
                return false;
